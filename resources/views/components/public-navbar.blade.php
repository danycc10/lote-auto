@php
    ...
    $whatsapp    = $whatsapp ?? \App\Models\Configuracion::obtener('contact.whatsapp', '5210000000000');
    $tagline     = \App\Models\Configuracion::obtener('branding.tagline', 'Autos financiados');
...
>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[68px] flex items-center justify-between gap-6">
        ...
        <a href="{{ $homeUrl }}" class="flex items-center gap-3 group min-w-0 shrink-0" aria-label="{{ config('app.name') }}">
            @if($logoUrl)
                <img src="{{ Storage::url($logoUrl) }}"
                     alt="{{ config('app.name') }}"
                     class="h-10 w-auto max-w-[120px] object-contain shrink-0">
            @else
                <div class="h-10 w-10 rounded-xl flex items-center justify-center shadow-lg shrink-0 transition"
            ...
            <div class="min-w-0">
                ...
                <p class="text-[11px] font-medium mt-0.5 leading-none" style="color: var(--color-secundario); opacity: 0.85">
                    {{ $tagline }}
                </p>
            </div>
        </a>
        ...
        <div class="hidden sm:flex items-center gap-2.5 shrink-0">
            <a href="https://wa.me/{{ $whatsapp }}?text={{ urlencode('Hola, quiero información sobre los autos disponibles') }}"
               ...
               class="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold text-white shadow-lg transition hover:opacity-90 active:scale-[0.97]"
    ...
    >
        <div class="max-w-7xl mx-auto px-4 py-4 space-y-1">
            ...
            <div class="pt-2 pb-1">
                <a href="https://wa.me/{{ $whatsapp }}?text={{ urlencode('Hola, quiero información sobre los autos disponibles') }}"
                   target="_blank" rel="noopener noreferrer"
                   class="flex w-full items-center justify-center gap-2 rounded-xl px-5 py-4 text-sm font-bold text-white transition hover:opacity-90 active:scale-[0.98]"
                   style="background-color: var(--color-secundario)">
