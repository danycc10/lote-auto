<div class="p-4 sm:p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @can('logs_financieros.ver')
                <a href="{{ route('admin.finanzas.logs-financieros') }}"
                   class="group bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all">
                    ...
                    <h2 class="text-sm font-semibold text-slate-900 group-hover:text-rose-700 transition-colors">Registro financiero</h2>
        ...
        @if(auth()->user()->hasAnyRole(['administrador', 'gerente']))
            <a href="{{ route('admin.sistema.apariencia') }}"
               class="group bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all">
                ...
                <h2 class="text-sm font-semibold text-slate-900 group-hover:text-indigo-700 transition-colors">Apariencia</h2>
                <p class="text-xs text-slate-500 mt-1">Personaliza colores, textos y datos del sitio público.</p>
            </a>

            <a href="{{ route('admin.sistema.landing') }}"
               class="group bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-violet-300 transition-all">
                <div class="h-10 w-10 rounded-lg bg-violet-50 flex items-center justify-center mb-4">
                    <svg class="h-5 w-5 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M11.25 4.533A9.707 9.707 0 006 3a9.735 9.735 0 00-3.25.555.75.75 0 00-.5.707v14.25a.75.75 0 001 .707A8.237 8.237 0 016 18.75c1.995 0 3.823.707 5.25 1.886V4.533zM12.75 20.636A8.214 8.214 0 0118 18.75c.966 0 1.89.166 2.75.47a.75.75 0 001-.708V4.262a.75.75 0 00-.5-.707A9.735 9.735 0 0018 3a9.707 9.707 0 00-5.25 1.533v16.103z"/>
                    </svg>
                </div>
                <h2 class="text-sm font-semibold text-slate-900 group-hover:text-violet-700 transition-colors">Plantillas del sitio</h2>
                <p class="text-xs text-slate-500 mt-1">Elige entre 5 diseños distintos para tu página pública.</p>
            </a>
        @endif
