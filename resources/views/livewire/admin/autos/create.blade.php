<div class="p-4 sm:p-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Nuevo auto</h1>
            <p class="text-sm text-slate-500 mt-0.5">Captura una nueva unidad para el inventario.</p>
        </div>
        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3.5 border-b border-slate-100 bg-slate-50/70">
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Sube varias imágenes y elige cuál será la portada.</p>
                </div>
                <div class="p-5 space-y-4">
                    ...
                    @error('imagenes') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                    @error('imagenes.*') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                    ...
                    @if(!empty($imagenes))
                        <div>
                            <p class="text-xs font-medium text-slate-500 uppercase tracking-wider mb-3">
                                Vista previa — {{ count($imagenes) }} imagen(es)
                            </p>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach($imagenes as $index => $img)
                                    @php
                                        $ext = strtolower($img->getClientOriginalExtension());
                                        $esPortada = (int) $portadaIndex === (int) $index;
                                    @endphp
                                    <div wire:key="preview-{{ $index }}"
                                        ...
                                        <div class="aspect-[4/3] bg-slate-100">
                                            @if(in_array($ext, ['jpg','jpeg','png','webp']))
                                                <img src="{{ $img->temporaryUrl() }}" alt="Imagen {{ $index + 1 }}"
                                        ...
                                        <div class="p-2.5 space-y-2">
                                            <div class="flex items-center justify-between gap-1">
                                                <p class="text-xs font-medium text-slate-700 truncate">Img {{ $index + 1 }}</p>
                                                @if($esPortada)
                    ...
                    @else
                        <div class="rounded-xl border border-slate-200 bg-slate-50 py-8 text-center">
                            <p class="text-sm text-slate-400">Aún no has cargado imágenes.</p>
                        </div>
                    @endif
        ...
        <div class="space-y-5 xl:sticky xl:top-[4.5rem] xl:self-start">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5 space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-slate-500">Imágenes</span>
                        <span class="text-xs font-semibold text-slate-900">{{ is_array($imagenes) ? count($imagenes) : 0 }}</span>
                    </div>
                </div>
            </div>
